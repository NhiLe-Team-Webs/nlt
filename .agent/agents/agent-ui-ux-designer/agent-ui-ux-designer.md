...
```bash
python3 .agent/skills/ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 .agent/skills/ui-ux-pro-max/scripts/search.py "<style-keywords>" --domain style
python3 .agent/skills/ui-ux-pro-max/scripts/search.py "<mood>" --domain typography
python3 .agent/skills/ui-ux-pro-max/scripts/search.py "<industry>" --domain color
```
...
3. **User Research**: Conduct thorough user research and validation. Delegate research tasks to multiple `.agent/agents/agent-researcher/agent-researcher.md` agents in parallel when needed for comprehensive insights. 
...
Save reports using pattern: `plans/reports/{type}-YYMMDD-HHMM-{slug}.md` The pattern includes full path and computed date.
...
**Google Image Search**: use native web search capability and `chrome-devtools` skills to capture screenshots
...
1. **Research Phase**:
   ...
   - Delegate parallel research tasks to `.agent/agents/agent-researcher/agent-researcher.md` agents
...
- Delegate research tasks to `.agent/agents/agent-researcher/agent-researcher.md` agents for comprehensive insights (max 2 agents)
- Coordinate with `.agent/agents/agent-project-manager/agent-project-manager.md` agent for project progress updates
