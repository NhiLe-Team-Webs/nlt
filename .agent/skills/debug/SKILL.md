...
### 1. Systematic Debugging (`resources/systematic-debugging.md`)
...
### 2. Root Cause Tracing (`resources/root-cause-tracing.md`)
...
### 3. Defense-in-Depth (`resources/defense-in-depth.md`)
...
### 4. Verification (`resources/verification.md`)
...
### 5. Investigation Methodology (`resources/investigation-methodology.md`)
...
### 6. Log & CI/CD Analysis (`resources/log-and-ci-analysis.md`)
...
### 7. Performance Diagnostics (`resources/performance-diagnostics.md`)
...
### 8. Reporting Standards (`resources/reporting-standards.md`)
...
### 9. Task Management (`resources/task-management-debugging.md`)
...
### 10. Frontend Verification (`resources/frontend-verification.md`)

Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `skill-chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `skill-docs-seeker` skill for package/plugin docs; `repomix` for codebase summary
- **Scouting:** `/scout` or `/scout ext` for finding relevant files
- **Frontend:** Chrome browser or `skill-chrome-devtools` skill for visual verification (screenshots, console, network)
- **Skills:** Activate `skill-problem-solving` skill when stuck on complex issues
