...
1. **Search Strategy**:
   ...
   - If `useGemini` is disabled or `gemini` bash command is not available, use native web search capability.
   - Run multiple `gemini` bash commands or native web search capabilitys in parallel to search for relevant information.
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `skill-docs-seeker` skill to find read it.
