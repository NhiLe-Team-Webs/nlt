...
2. **Fallback to mcp-manager subagent** (if Gemini CLI unavailable):
   - Use `.agent/agents/agent-mcp-manager/agent-mcp-manager.md` instructions to discover and execute tools
   - If the subagent got issues with the scripts of `skill-mcp-management` skill, use `skill-mcp-builder` skill to fix them
