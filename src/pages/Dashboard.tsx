...
const CULTURE_QUESTIONS = [
  {
    q: "Tại sao cần xem hết video của Chị Nhi trước khi đặt câu hỏi?",
    a: ["Video rất hay và thú vị", "Thể hiện sự quan tâm", "Câu hỏi của bạn có thể đã được trả lời rồi", "Để tạo mối quan hệ với mentor"],
    correct: 2,
  },
  {
    q: "Tại sao cần gửi tin nhắn phản hồi khi nhận thông báo?",
    a: ["Thể hiện sự chuyên nghiệp", "Thể hiện sự tôn trọng và xác nhận đã nhận thông tin", "Để trông thân thiện hơn", "Để cho thấy bạn đang online"],
    correct: 1,
  },
  {
    q: "Tại sao thành viên NhiLe Team phải có tên và ảnh đại diện rõ ràng?",
    a: ["Để nhận diện, tôn trọng đồng đội và tự tôn trọng bản thân", "Để tăng tương tác", "Để nổi bật trong nhóm", "Cho vui"],
    correct: 0,
  },
  {
    q: 'Tại sao tránh dùng những từ vô nghĩa như "sẽ", "hi vọng", "cố gắng", "thử", "mong"?',
    a: ["Team yêu cầu giao tiếp ngắn gọn", "Chúng có vẻ không chân thật", "Chúng cản trở khả năng quyết đoán của bản thân", "Chúng tạo ra căng thẳng không cần thiết"],
    correct: 2,
  },
  {
    q: "Điều gì bị cấm nhất trong NhiLe Team?",
    a: ["Trễ deadline", "Bất đồng và ý kiến trái chiều", "Vắng mặt trong các buổi họp", "Drama, gossip và không hỗ trợ thành viên mới"],
    correct: 3,
  },
  {
    q: "Tại sao NhiLe Team thực hiện feedback cho nhau?",
    a: ["Tự thể hiện bản thân", "Gây ấn tượng với người khác", "Giúp thành viên phát triển liên tục", "Tạo sự cạnh tranh lành mạnh"],
    correct: 2,
  },
  {
    q: "Tại sao NhiLe Team có văn hóa đóng quỹ?",
    a: ["Làm giàu cho nhóm về mặt tài chính", "Khuyến khích làm việc chăm chỉ hơn", "Đối mặt với hậu quả của lỗi lầm và học trách nhiệm", "Hỗ trợ các dự án mới"],
    correct: 2,
  },
  {
    q: "NhiLe Team được thành lập để làm gì?",
    a: ["Tạo cộng đồng trực tuyến chuyên nghiệp", "Xây dựng môi trường tích cực cho người Việt học hỏi, kết nối và phát triển", "Chia sẻ kiến thức và nâng cao hình ảnh cá nhân", "Đào tạo các nhà lãnh đạo xuất sắc"],
    correct: 1,
  },
  {
    q: "Điều quan trọng nhất khi làm việc nhóm là gì?",
    a: ["Hoàn thành phần của mình", "Làm nhiều hơn để được ghi nhận", "Hỗ trợ nhau và duy trì thái độ tích cực", "Làm việc im lặng"],
    correct: 2,
  },
  {
    q: "Khi có việc cá nhân, bạn nên làm gì?",
    a: ["Không cần báo cáo", "Im lặng", "Tắt thiết bị", "Thông báo cho leader hoặc nhóm trước"],
    correct: 3,
  },
];
...
const Dashboard = () => {
  ...
  const openQuiz = (type: "skill" | "culture") => {
  ...
  const handleAnswer = (idx: number) => {
    ...
    if (currentQuestionIndex < totalQuestions - 1) {
      ...
      // Score with the full answers array (state not updated yet, use newAnswers)
      const score = newAnswers.filter((ans, i) => ans === currentQuestions[i].correct).length;
      ...
      if (passed) {
        setTimeout(() => {
          closeQuiz();
          setCurrentStep(activeQuizType === "culture" ? 2 : 3);
  ...
  const steps = [
      ...
      customContent: (
        <div className="mt-6 space-y-5 transition-all duration-300">
          ...
          <button
            ...
            className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-black text-sm shadow-lg shadow-blue-600/20 hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2 w-full sm:w-auto justify-center"
          >
    ...
    },
    {
      ...
      desc: "Thực hiện bài test để chúng mình hiểu sự phù hợp nha.",
                    ...
                    return (
                      <button
                        ...
                        className={`aspect-square flex items-center justify-center rounded-xl text-xs font-black transition-all relative group/date
                          ${isSelected ? "bg-[#3B82F6] text-white shadow-[0_0_20px_rgba(59,130,246,0.5)] scale-110 z-10"
                            : isHighDemand ? "bg-blue-50 text-[#3B82F6] border border-blue-200"
                            : isAvailable ? "text-[#1D1D1F] hover:bg-blue-50 hover:text-[#3B82F6]"
                            : isFullyBooked ? "text-red-300 bg-red-50/30 cursor-not-allowed line-through"
                            : "text-gray-200 cursor-not-allowed"}
                          ${isToday ? "ring-2 ring-inset ring-[#3B82F6]/30" : ""}`}
                      >
                        {date}
                        {isHighDemand && !isSelected && <div className="absolute top-1 right-1 w-1 h-1 bg-orange-400 rounded-full animate-pulse"></div>}
  ...
  return (
    <div className={`min-h-[100dvh] bg-[#F9F9F9] flex flex-col md:flex-row font-sans transition-opacity duration-1000 ${isLoaded ? "opacity-100" : "opacity-0"}`}>
      ...
      <aside className={`fixed inset-0 z-[90] md:relative md:flex md:w-80 bg-white border-r border-gray-100 p-8 flex-col justify-between h-screen transition-transform duration-500 ${isMobileMenuOpen ? "translate-y-0" : "-translate-y-full md:translate-y-0"}`}>
        ...
        <button onClick={handleLogout} className="flex items-center gap-2 text-gray-400 hover:text-red-500 transition-all text-sm font-black mt-10 group pb-8 md:pb-0">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-8">
            <div className="space-y-2 text-center sm:text-left">
              ...
              <p className="text-gray-500 text-sm sm:text-base font-bold max-w-xl leading-relaxed mx-auto sm:mx-0">
                Chúng mình đã chuẩn bị sẵn mọi thứ — hoàn tất vài bước nhỏ để bắt đầu hành trình cùng{" "}
                <span className="text-blue-600">NhiLe Team</span> nha!
              </p>
            </div>
                ...
                <div key={step.id} className={`relative z-10 flex gap-5 sm:gap-8 p-6 sm:p-8 rounded-[2.5rem] transition-all duration-500 ${isActive ? "bg-white shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-50" : "bg-transparent"}`}>
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-sm transition-all duration-500
                    ${step.status === "completed" ? "bg-[#10B981] text-white"
                      : isActive
                        ? step.id === 5 ? "bg-gradient-to-br from-[#FB923C] to-[#F59E0B] text-white ring-8 ring-orange-500/10 animate-bounce-subtle"
                  ...
                  <div className={`flex-1 space-y-1.5 pt-1 text-left transition-opacity duration-500 ${step.status === "locked" ? "opacity-30" : "opacity-100"}`}>
                    <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${step.status === "completed" ? "text-[#10B981]" : isActive ? "text-blue-600" : "text-gray-400"}`}>
                      Bước {step.id}
                    </span>
      ...
      {isQuizModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            {!showQuizResult ? (
              <>
                {/* Question header — purple gradient */}
                <div className="bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] p-7 sm:p-8 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-white/70 uppercase tracking-widest">
                      {activeQuizType === "culture" ? "Test Văn Hoá" : "Test Năng Lực"}
                    </span>
                    <button onClick={closeQuiz} className="text-white/60 hover:text-white transition-colors">
                      <X size={18} />
                    </button>
                  </div>
                ...
                {/* Timer bar */}
                <div className="px-7 sm:px-8 pt-5 space-y-1">
                  ...
                  <div className="h-2.5 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-1000 ${timer <= 10 ? "bg-red-500" : "bg-gradient-to-r from-orange-400 to-yellow-400"}`}
                ...
                {/* Options */}
                <div className="px-7 sm:px-8 py-5 space-y-3">
                  {currentQuestions[currentQuestionIndex].a.map((ans, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleAnswer(idx)}
                      className="w-full flex items-center gap-4 p-4 border-2 border-gray-100 rounded-2xl font-bold text-sm text-[#1D1D1F] hover:border-[#6366F1] hover:bg-indigo-50 hover:text-[#6366F1] transition-all active:scale-[0.98] text-left"
                    >
                      <span className="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center text-xs font-black text-gray-500 shrink-0">
                        {OPTION_LABELS[idx]}
                      </span>
                      {ans}
                    </button>
                ...
                {/* Progress */}
                <div className="px-7 sm:px-8 pb-7 sm:pb-8 space-y-2">
              ...
              /* Result screen */
              <div className="p-10 sm:p-12 text-center space-y-6 animate-in zoom-in duration-300">
                {quizPassed ? (
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-gray-500 font-bold">
                        Bạn đúng <span className="text-green-600 font-black">{quizScore}/{totalQuestions}</span> câu — Đủ điều kiện sang bước tiếp theo!
                      </p>
                    </div>
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-gray-500 font-bold">
                        Bạn đúng <span className="text-red-500 font-black">{quizScore}/{totalQuestions}</span> câu — Cần đúng ít nhất{" "}
                        <span className="font-black text-[#1D1D1F]">{PASS_SCORE}/{totalQuestions}</span> để qua.
                      </p>
